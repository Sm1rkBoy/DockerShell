            ...
            for i in "${!containers[@]}"; do
                if [ "${selected[$i]}" -eq 1 ]; then
                    ...
                    if is_container_running "$container_name"; then
                        ...
                        # 动态调用安装函数
                        script_url="$GITHUB_SCRIPTS_BASE/$container_name.sh"
                        if curl -s --head "$script_url" | grep -q "HTTP/.*200"; then
                            echo "正在加载 $container_name 安装脚本..."
                            # 下载并加载脚本（使函数在当前 Shell 可用）
                            ...
                            if declare -f "$install_function" > /dev/null; then
                                ...
                                $install_function  # 调用安装函数
                            else
                                echo "错误: $install_function 未在脚本中定义。"
                            fi
                        else
                            echo "未知容器: $container_name（未找到安装脚本 $script_url）。"
                        fi
                    fi
                fi
            done
